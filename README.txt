...
*Django 1.4-1.5
*Django-Rest-Framework (2.X the more current the better)
**Ember.js and its associated dependencies are not technically required to use 
this package though some of the results of its use will remain hidden until 
both Ember.js and Ember-data.js are engaged.

On-Fire is a project initiated by Steve Kane and Peter Chen of 4-south Studios to 
solve integration problems between Ember.js and Django (with Django-Rest-Framework).  
The apps this package aims to help a user build are apps that live primarily in 
javascript on the client-side but need a robust back-end to handle validation, 
serving json, performing queries etc.
...
1.5) We are looking to add sideloading support to this paradigm as Ember already 
...
Toran Billups has an excellent django-rest-framework/ember adapter written that molds 
